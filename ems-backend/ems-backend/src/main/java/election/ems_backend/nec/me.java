...
//import election.ems_backend.nec.NecResultRecomputeEvent;
...
////                .orElseThrow(() -> new ResponseStatusException(NOT_FOUND, "Submission not found"));
////
////        // ✅ Only NEC admins should do this (enforce in AuthorizationService in your real code)
////        SystemUser actor = userRepo.findById(req.getActorUserId())
////                .orElseThrow(() -> new ResponseStatusException(NOT_FOUND, "Actor not found"));
////
////        // ✅ Allow amending VERIFIED/REJECTED (and optionally PENDING)
////        if (s.getStatus() != VoteStatus.VERIFIED && s.getStatus() != VoteStatus.REJECTED && s.getStatus() != VoteStatus.PENDING) {
////            throw new ResponseStatusException(BAD_REQUEST, "Only VERIFIED/REJECTED/PENDING submissions can be amended");
////        }
////
////        PollingPlace place = s.getPollingPlace();
...
////                .orElseThrow(() -> new ResponseStatusException(BAD_REQUEST, "Polling place not allocated for this election"));
////
////        // Merge votes (same approach as update)
////        Map<String, Integer> mergedVotes = mergeCandidateVotes(s.getCandidateVotes(), req.getCandidateVotes());
////        if (mergedVotes.size() > MAX_CANDIDATE_KEYS) {
////            throw new ResponseStatusException(BAD_REQUEST, "Too many candidate entries");
////        }
...
////        int unused = (req.getUnusedBallots() != null) ? req.getUnusedBallots() : nzInt(s.getUnusedBallots());
////
////        // ✅ Full validation (because this is a real correction, not a draft)
////        validateCandidateVotes(
////                s.getOrganization().getOrgId(),
////                s.getElection().getElectionId(),
////                s.getContestId(),
...
////        s.setCandidateVotes(mergedVotes);
////
////        // ✅ BEST PRACTICE: force re-verify
////        s.setStatus(VoteStatus.PENDING);
////        s.setVerifiedBy(null);
////        s.setDateVerified(null);
////
////        String existing = (s.getComments() == null ? "" : (s.getComments() + " "));
////        s.setComments(existing + "[amended] " + req.getReason().trim());
////
////        // ✅ Update hash because content changed
////        s.setSubmissionHash(buildSubmissionPayloadHash(s));
////
////        // ✅ Flush so recompute sees new status + values
////        VoteSubmission saved = voteSubmissionRepository.saveAndFlush(s);
////
////        // ---- ledger/sign (same pattern as update) ----
////        String payloadHash = buildSubmissionPayloadHash(saved);
////        Map<String, Object> ledgerRes = jdbc.queryForMap(
////                "SELECT * FROM fn_log_ledger_and_update_submission(?, ?, ?, ?)",
////                "VOTE_SUBMISSION_AMEND",
...
////                signResult.signature(), signResult.keyId(), saved.getSubmissionId());
////
////        // ✅ Trigger VoteTally recompute (unchanged system)
////        final RecomputeEvent recomputeEvent = new RecomputeEvent(
////                this,
////                saved.getOrganization().getOrgId(),
////                saved.getElection().getElectionId(),
...
////        }
////
////        // ✅ Trigger NEC recompute to CLEAR derived rows (because now PENDING)
////        // This will delete nec_result if verifiedCount==0
////        // ✅ NEW: pass submission comments into history.user_note (no other logic changes)
////        necResultService.recomputeFromSubmissionWithNotes(
////                saved.getSubmissionId(),
////                actor.getUserId(),
////                saved.getComments()
...
////                ActivityType.VOTE_UPDATED,
////                "vote_submission",
////                "Amended submission (reset to PENDING): " + saved.getSubmissionId() + " reason=" + req.getReason()
////        );
////
////        return mapper.toDTO(saved);
////    }
...
//    @Override
//    @Transactional
//    public void delete(UUID id, String reason, UUID deletedByUserId) {
//
//        if (id == null) throw new ResponseStatusException(BAD_REQUEST, "id is required");
//        if (reason == null || reason.trim().isEmpty())
//            throw new ResponseStatusException(BAD_REQUEST, "reason is required");
//        if (deletedByUserId == null)
//            throw new ResponseStatusException(BAD_REQUEST, "deletedByUserId is required");
//
...
//                .orElseThrow(() -> new ResponseStatusException(NOT_FOUND, "Submission not found"));
//
//        // capture scope BEFORE delete
//        UUID submissionId = s.getSubmissionId();
//
...
//
//        UUID orgId = s.getOrganization().getOrgId();
//        UUID electionId = s.getElection().getElectionId();
//        UUID contestId = s.getContestId();
//
//        // centerId can be on submission OR derived from place->center
//        UUID centerId = null;
...
//        }
//
//        // Optional: who to notify (agent)
//        UUID agentUserId = (s.getAgent() != null ? s.getAgent().getUserId() : null);
//
//        // build note for NEC history
//        final String userNote =
//                "DELETE_SUBMISSION: submissionId=" + submissionId + " | reason=" + cleanReason;
//
//        // ✅ HARD delete
//        voteSubmissionRepository.deleteById(id);
//
//        // ✅ Audit (use the method you already use elsewhere)
//        // Pick ONE that exists in your codebase:
//        auditLogService.logDelete(
//                orgId,
//                deletedByUserId,
//                "VoteSubmission",
//                "Deleted submission: " + submissionId + " | reason: " + cleanReason
//        );
//
//        // ✅ Notify (optional)
//        if (agentUserId != null) {
...
//                    NotificationType.VOTE,
//                    "Submission Deleted",
//                    "Your vote submission was deleted. Reason: " + cleanReason,
//                    "vote_submission", submissionId,
//                    NotificationPriority.LOW, DeliveryMethod.IN_APP
//            );
//        }
//
//        // ✅ Recompute NEC AFTER COMMIT so the delete is visible to recompute SQL
//        final UUID fOrgId = orgId;
//        final UUID fElectionId = electionId;
//        final UUID fContestId = contestId;
//        final UUID fCenterId = centerId;
//        final UUID fDeletedBy = deletedByUserId;
//        final String fUserNote = userNote;
//
//        if (TransactionSynchronizationManager.isSynchronizationActive()) {
//            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
//                @Override
...
//                        );
//                    } catch (Exception ex) {
//                        log.warn("NEC recompute failed after delete (ignored): {}", ex.getMessage());
//                    }
//                }
//            });
//        } else {
...
////        UUID electionId = s.getElection().getElectionId();
////        UUID contestId = s.getContestId();
////        UUID actorUserId = (s.getAgent() != null ? s.getAgent().getUserId() : null);
////
////        UUID centerId = null;
...
////        }
////
////        // ✅ HARD delete (you said it is removed from DB)
////        voteSubmissionRepository.deleteById(id);
////
...
////        );
////
////        // Notify includes reason (optional)
////        notify(
////                orgId,
////                actorUserId,
////                NotificationType.VOTE,
////                "Submission Deleted",
////                "Your vote submission for " + s.getPollingCenter().getCenterName() + " was deleted. Reason: " + cleanReason,
////                "vote_submission", submissionId,
////                NotificationPriority.LOW, DeliveryMethod.IN_APP
////        );
////
////        // effectively-final copies for afterCommit
////        final UUID fOrgId = orgId;
////        final UUID fElectionId = electionId;
////        final UUID fContestId = contestId;
////        final UUID fCenterId = centerId;
////        final UUID fActorUserId = actorUserId;
////        final UUID fSubmissionId = submissionId;
////        final String fReason = cleanReason;
////
////        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
////            @Override
////            public void afterCommit() {
////                try {
////                    // ✅ Recompute and record reason into history notes
////                    necResultService.recomputeForCenterContestWithNotes(
////                            fOrgId,
////                            fElectionId,
////                            fContestId,
////                            fCenterId,
////                            fActorUserId,
////                            "DELETE_SUBMISSION: submissionId=" + fSubmissionId + " | reason=" + fReason
////                    );
////                } catch (Exception ex) {
////                    log.warn("NEC recompute failed after delete (ignored): {}", ex.getMessage());
////                }
