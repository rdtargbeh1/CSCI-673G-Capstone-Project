...
import SubmissionResubmitPage from "../SubmissionResubmitPage";

import SubmissionReopenPage from "../SubmissionReopenPage";
// ============================================================================
// TYPES
// ============================================================================
...
type SubmissionRoute =
  ...
  | { type: "resubmit"; submissionId: string }
  | { type: "reopen"; submissionId: string }
  | { type: "submit-draft"; submissionId: string }
  | { type: "unknown" };
...
export default function SubmissionsTab() {
  ...
  const route = useMemo<SubmissionRoute>(() => {
    ...
    const resubmitMatch = pathname.match(
    ...
    if (resubmitMatch?.[1]) {
      return {
        type: "resubmit",
        submissionId: decodeRouteValue(resubmitMatch[1]),
      };
    }
    ...
    const reopenMatch = pathname.match(/\/submissions\/([^/]+)\/reopen\/?$/);

    if (reopenMatch?.[1]) {
      return {
        type: "reopen",
        submissionId: decodeRouteValue(reopenMatch[1]),
      };
    }
    ...
    const submitDraftMatch = pathname.match(
      /\/submissions\/([^/]+)\/submit-draft\/?$/,
    );

    if (submitDraftMatch?.[1]) {
      return {
        type: "submit-draft",
        submissionId: decodeRouteValue(submitDraftMatch[1]),
      };
    }
  ...
  if (route.type === "resubmit") {
    return <SubmissionResubmitPage submissionId={route.submissionId} />;
  }
  ...
  if (route.type === "reopen") {
    return <SubmissionReopenPage submissionId={route.submissionId} />;
  }
