...
import {
  searchCenterStatsParty,
  type CenterStatsPartyRow,
} from "../../../../../../shared/services/stats/centerStatsPartyService";
...
/** ✅ NEW: "reported/places" like 1/3, 3/3 */
function fmtRepOverPlaces(reported: any, total: any) {
  ...
  // if no allocation / no places, show dash like other columns
  if (rr === 0 && tt === 0) return "—";
  return `${rr}/${tt}`;
}
...
export default function PartyTotalsCentersPage() {
  ...
  const countiesQ = useQuery<CountyDto[]>({
    ...
    // ✅ SYSTEM requires orgId; TENANT/NEC can load without it
    enabled: mode === "SYSTEM" ? Boolean(orgId) : true,
    queryFn: async () =>
      (
        await fetchCounties({
          page: 0,
  ...
  const districtsQ = useQuery<DistrictDto[]>({
    enabled: Boolean(countyId) && (mode === "SYSTEM" ? Boolean(orgId) : true),
    queryKey: ["districts", "by-county", mode, orgId, countyId],
    // NOTE: keep signature as 1 arg to avoid your "Expected 1 arguments, but got 2."
  ...
  const centersQ = useQuery<PollingCenterDto[]>({
    enabled: (Boolean(countyId) || Boolean(districtId)) && (mode === "SYSTEM" ? Boolean(orgId) : true),
    queryKey: ["polling-centers", "filtered", mode, orgId, countyId, districtId],
    queryFn: async () => {
  ...
  const statsEnabled = useMemo(() => {
    ...
    // ✅ match backend: SYSTEM must supply orgId; TENANT/NEC can derive
    if (mode === "SYSTEM") return Boolean(orgId);
    return true;
  }, [electionId, mode, orgId]);

  // ✅ send orgId only for SYSTEM (avoid accidental 403 mismatch)
  const orgIdParam = orgId || undefined;
  ...
  const rows: CenterStatsPartyRow[] = q.data?.content ?? [];

  if (mode === "SYSTEM" && !orgId) {
    return (
      <Panel title="Party Totals • Centers">
        <PlaceholderNote
          title="Select an Organization first"
          bullets={[
            "SYSTEM mode requires org selection (backend expects orgIdParam).",
    ...
    >
      ...
      {q.isLoading ? (
        ...
        <PlaceholderNote
          ...
          bullets={[
            "This election may not have verified tallies yet for the selected filters.",
            ...
            <table className="min-w-full table-fixed text-base">
              <thead className="bg-slate-50 text-slate-700">
                <tr>
                  ...
                  <Th className="w-[110px]"><SortBtn onClick={() => toggleSort("centerCode")}>Code</SortBtn></Th>
                  <Th className="w-[210px]"><SortBtn onClick={() => toggleSort("centerName")}>Center</SortBtn></Th>

                  <Th className="w-[80px] text-right"><SortBtn onClick={() => toggleSort("registeredVoters")}>Reg</SortBtn></Th>
                  <Th className="w-[80px] text-right"><SortBtn onClick={() => toggleSort("ballotsIssued")}>Issued</SortBtn></Th>
                  <Th className="w-[80px] text-right"><SortBtn onClick={() => toggleSort("ballotsCast")}>Cast</SortBtn></Th>
                  <Th className="w-[80px] text-right"><SortBtn onClick={() => toggleSort("validVotes")}>Valid</SortBtn></Th>

                  {/* ✅ NEW column */}
                  <Th className="w-[80px] text-right">Valid%</Th>

                  <Th className="w-[80px] text-right"><SortBtn onClick={() => toggleSort("invalidTotal")}>Invalid</SortBtn></Th>

                  <Th className="w-[80px] text-right"><SortBtn onClick={() => toggleSort("turnoutPct")}>Turn%</SortBtn></Th>
                  <Th className="w-[80px] text-right"><SortBtn onClick={() => toggleSort("invalidPct")}>Invalid%</SortBtn></Th>

                  <Th className="w-[70px] text-right"><SortBtn onClick={() => toggleSort("placesTotal")}>Places</SortBtn></Th>

                  <Th className="w-[80px] text-right">
                    <SortBtn onClick={() => toggleSort("placesReported")}>Reptd</SortBtn>
                  </Th>

                  <Th className="w-[85px] text-right"><SortBtn onClick={() => toggleSort("placesReportingPct")}>Reptd%</SortBtn></Th>

                  <Th className="w-[70px] text-right"><SortBtn onClick={() => toggleSort("centerStarted")}>Start</SortBtn></Th>
                  <Th className="w-[70px] text-right"><SortBtn onClick={() => toggleSort("centerPartial")}>Partial</SortBtn></Th>
                  <Th className="w-[80px] text-right"><SortBtn onClick={() => toggleSort("centerCompleted")}>Done</SortBtn></Th>

                  <Th className="w-[70px] text-right"><SortBtn onClick={() => toggleSort("hasPlaceAllocation")}>Alloc</SortBtn></Th>
                </tr>
              </thead>
                ...
                {rows.map((r: any) => {
                  ...
                  return (
                    <tr key={r.centerId} className="hover:bg-slate-50">
                      ...
                      <Td className="text-right tabular-nums whitespace-nowrap">{fmtNum(r.ballotsIssued)}</Td>
                      ...
                      <Td className="text-right tabular-nums whitespace-nowrap">{fmtNum(r.placesTotal)}</Td>
                      ...
                      <Td className="text-right tabular-nums whitespace-nowrap">{fmtPct(r.placesReportingPct)}</Td>

                      <Td className="text-right tabular-nums whitespace-nowrap">{fmtNum(r.centerStarted)}</Td>
                      <Td className="text-right tabular-nums whitespace-nowrap">{fmtNum(r.centerPartial)}</Td>
                      ...
                      <Td className="text-right tabular-nums whitespace-nowrap">{fmtNum(r.hasPlaceAllocation)}</Td>
                    </tr>
                  );
                })}
              </tbody>
