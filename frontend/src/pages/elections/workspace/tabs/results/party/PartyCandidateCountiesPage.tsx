...
// ✅ FILE: src/pages/elections/workspace/tabs/results/party/PartyCandidateCountiesPage.tsx
...
import {
  searchCandidateCountyStatsParty,
  type CandidateCountyStatsPartyRow,
...
export default function PartyCandidateCountiesPage() {
  ...
  // ✅ when candidate is selected: county-first then highest votes
  useEffect(() => {
    if (!candidateId) return;
    setSort(["countyName,asc", "candidateVotes,desc", "voteSharePct,desc"]);
  ...
  // ----------------------------
  // 2) Counties list (for county filter)
  // ✅ pass orgId for SYSTEM
  // ----------------------------
  ...
  const rows: CandidateCountyStatsPartyRow[] = q.data?.content ?? [];
  ...
  if (!orgId) {
    return (
      <Panel title="Party Results • Candidates by County">
        <PlaceholderNote
  ...
  return (
    <Panel
      title="Party Results • Candidates by County"
      right={
    ...
    >
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <select
          ...
          title={!contestId ? "Select a contest first to filter by candidate" : "Filter counties by a single candidate"}
        >
        ...
        {countyId ? (
          <button type="button" onClick={clearCounty} className="h-9 rounded-xl border bg-white px-3 text-base font-extrabold hover:bg-slate-50">
            Clear County
          </button>
        ) : null}
      ...
      {q.isLoading ? (
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-base text-slate-700">
          Loading candidate county results…
        </div>
      ) : q.isError ? (
        ...
        <PlaceholderNote
          ...
          bullets={[
            ...
            "Try removing filters, or select a contest before filtering by candidate.",
          ]}
        />
      ) : (
        <>
                  ...
                  return (
                    <tr key={`${r.contestId}-${r.countyId}-${r.candidateId}`} className="hover:bg-slate-50">
                      ...
                      <Td className="truncate" title={r.partyName ?? "INDEPENDENT"}>
                        {r.partyName ?? "INDEPENDENT"} {r.abbreviation ? `(${r.abbreviation})` : ""}
                      </Td>
