...
// ✅ FILE: src/pages/elections/workspace/tabs/results/party/PartyCandidateElectionPage.tsx
...
import {
  searchCandidateElectionStatsParty,
  type CandidateElectionStatsPartyRow,
} from "../../../../../../shared/services/stats/candidateElectionStatsPartyService";
...
export default function PartyCandidateElectionPage() {
  ...
  // ✅ SYSTEM org scope (match PartyCandidateCentersPage)
  ...
  if (!orgId) {
    return (
      <Panel title="Party Results • Candidates (Election)">
        <PlaceholderNote
  ...
  const candidatesQ = useQuery({
    queryKey: ["stats", "party", "candidate-election", "candidates", orgId, electionId, contestId],
    enabled: Boolean(electionId) && Boolean(orgId) && Boolean(contestId),
    queryFn: async () =>
      searchCandidateElectionStatsParty({
        orgId,
  ...
  const rows: CandidateElectionStatsPartyRow[] = q.data?.content ?? [];
  ...
  return (
    <Panel
      title="Party Results • Candidates (Election)"
      right={
    ...
    >
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <select
          ...
          className="h-9 min-w-[260px] rounded-xl border bg-white px-3 text-sm font-bold disabled:bg-slate-50"
      ...
      {q.isLoading ? (
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-base text-slate-700">
          Loading candidate election results…
        </div>
      ) : q.isError ? (
        ...
        <PlaceholderNote
          ...
          bullets={[
            ...
            "Try selecting a contest, or remove the candidate filter.",
          ]}
        />
      ) : (
        <>
