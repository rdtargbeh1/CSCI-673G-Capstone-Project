...
// ✅ FILE: src/pages/elections/workspace/tabs/results/official/totals/OfficialCandidateDistrictPage.tsx
...
import {
  searchCandidateDistrictStatsOfficial,
  type CandidateDistrictStatsOfficialRow,
} from "../../../../../../shared/services/stats/candidateDistrictStatsOfficialService";
...
export default function OfficialCandidateDistrictsPage() {
  ...
  useEffect(() => {
    ...
    setSort(["districtName,asc", "candidateVotes,desc", "voteSharePct,desc", "candidateName,asc"]);
  ...
  // Districts by county (optional)
  const districtsQ = useQuery({
      queryKey: ["districts", "by-county", countyId],
      enabled: Boolean(countyId),
      queryFn: async () => (await fetchDistrictsByCounty(String(countyId))) as DistrictDto[],
      staleTime: 60_000,
      retry: 1,
    });
...
const districts = districtsQ.data ?? [];
  ...
  const rows: CandidateDistrictStatsOfficialRow[] = q.data?.content ?? [];

  // Candidate options (only after contest selected)
  ...
  if (!electionId) {
    return (
      <Panel title="Official Results • Candidates by District">
  ...
  // handlers
  const onContestChange = (v: string) =>
    setSP(searchParams, setSearchParams, { contestId: v || undefined, candidateId: undefined }, { resetPage: true });

  const onCountyChange = (v: string) =>
    setSP(searchParams, setSearchParams, { countyId: v || undefined, districtId: undefined }, { resetPage: true });

  const onDistrictChange = (v: string) =>
    setSP(searchParams, setSearchParams, { districtId: v || undefined }, { resetPage: true });

  const onCandidateChange = (v: string) =>
    setSP(searchParams, setSearchParams, { candidateId: v || undefined }, { resetPage: true });
  ...
  return (
    <Panel
      title="Official Results • Candidates by District"
      right={
    ...
    >
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <select
          ...
          disabled={!countyId || districtsQ.isFetching}
          title={!countyId ? "Select a county first" : "District (optional)"}
        >
          <option value="">{!countyId ? "Select county first…" : "All districts"}</option>
          {districts.map((d: any) => (
        ...
        {candidateId ? (
          <button type="button" onClick={clearCandidate} className="h-9 rounded-xl border bg-white px-3 text-base font-extrabold hover:bg-slate-50">
            Clear Candidate
          </button>
        ) : null}
      ...
      {q.isLoading ? (
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
          Loading official candidate district results…
        </div>
      ) : q.isError ? (
            ...
            <table className="min-w-full table-fixed text-base">
              <thead className="bg-slate-50 text-slate-700">
                <tr>
                  ...
                  <Th className="w-[190px]"><SortBtn onClick={() => toggleSort("districtName")}>District</SortBtn></Th>
              ...
              <tbody>
                {rows.map((r: any) => {
                  const isWin = Boolean(r.isDistrictWinner) || Number(r.rankInDistrict) === 1;
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`h-9 rounded-xl border bg-white px-3 text-base font-extrabold ${
      ...
      <div className="mt-3 text-lg text-slate-500">
        Data source: <code>v_candidate_district_stats_official</code>
      </div>
    </Panel>
  );
}
...
function Th(props: React.ThHTMLAttributes<HTMLTableCellElement>) {
  return <th {...props} className={`border-b px-2 py-1 text-left text-lg font-extrabold ${props.className ?? ""}`} />;
}
